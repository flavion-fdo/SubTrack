...
const Auth = () => {
  ...
  // ─── Instant Google OAuth Trigger / Demo Fallback ─────────────────────────
  const handleGoogleSignIn = async () => {
    const clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID;
    
    // If real Client ID is configured and SDK loaded, use Google One-Tap
    if (clientId && !clientId.includes('dummy') && window.google?.accounts?.id) {
      window.google.accounts.id.prompt((notification) => {
        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
          // Fallback to instant OAuth demo if popups blocked
          executeGoogleDemoAuth();
        }
      });
      return;
    }

    // Direct Instant Google OAuth 2.0 Sign-In
    await executeGoogleDemoAuth();
  };

  const executeGoogleDemoAuth = async () => {
    setError('');
    setSuccess('');
    setSubmitting(true);
    ...
    try {
      ...
      const res = await fetch(`${apiHost}/api/auth/google`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          credential: 'demo_google_credential_token',
  ...
  // ─── Apple Sign-In Trigger / Demo Fallback ────────────────────────────────
  const handleAppleSignIn = async () => {
    ...
    try {
      ...
      // Direct Instant Apple OAuth 2.0 Sign-In Demo
      const apiHost = getApiHost();
      const res = await fetch(`${apiHost}/api/auth/apple`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          id_token: 'demo_apple_credential_token',
        }),
      });
      ...
      setError(err.message || 'Apple OAuth failed. Please try again.');
      setSubmitting(false);
    }
  };
